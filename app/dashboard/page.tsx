  ...
  try {
    ...
    const avgSentimentScore =
      scoredCalls.length > 0
        ? scoredCalls.reduce(
            (acc, c) => acc + (sentimentScores[c.sentiment] ?? 0),
            0
          ) / scoredCalls.length
        : 0;
...
export default async function DashboardPage() {
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: stats.consultationBreakdown.scheduled.toString(),
    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="mb-8 animate-slide-up">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Real-time intelligence from your AI voice agent
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {metrics.map((m, i) => (
          ...
          >
            ...
            <div className="text-3xl font-semibold text-gray-900 tracking-tight mb-0.5">
              {m.value}
            </div>
          ...
          <tbody>
            {stats.recentCalls.length === 0 ? (
              ...
              stats.recentCalls.map((call, i) => (
                <tr
                  key={call.call_id}
                  className={`border-b border-black/[0.04] hover:bg-black/[0.02] transition-colors ${i === stats.recentCalls.length - 1 ? "border-b-0" : ""}`}
                >
                  <td className="px-5 py-3.5 font-mono text-[12px] text-gray-600">
                    {call.from_number || "—"}
                  </td>
                  <td className="px-5 py-3.5 text-gray-700 font-medium">
                    {call.lead_name || <span className="text-gray-400">Unknown</span>}
                  </td>
