...
async function getDashboardStats() {
  try {
    ...
    const sentimentScores: Record<string, number> = { positive: 1, Positive: 1, neutral: 0, Neutral: 0, negative: -1, Negative: -1 };
    const scoredCalls = calls.filter((c) => sentimentScores[c.sentiment] !== undefined);
    const avgScore = scoredCalls.length > 0 ? scoredCalls.reduce((a, c) => a + (sentimentScores[c.sentiment] ?? 0), 0) / scoredCalls.length : 0;
    const avgSentimentLabel = avgScore > 0.2 ? "Positive" : avgScore < -0.2 ? "Negative" : "Neutral";
...
function SentimentBadge({ sentiment }: { sentiment: string }) {
  ...
  const cls = s === "positive" ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" : s === "negative" ? "bg-red-50 text-red-700 ring-1 ring-red-200" : "bg-gray-100 text-gray-600 ring-1 ring-gray-200";
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium ${cls}`}>{sentiment || "Unknown"}</span>;
}
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {stats.recentCalls.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-gray-400">No calls recorded yet.</td></tr>
            ) : stats.recentCalls.map((call) => (
              <tr key={call.call_id} className="border-b border-black/[0.04] hover:bg-black/[0.02] transition-colors">
                ...
                <td className="px-5 py-3.5"><span className="inline-flex items-center gap-1"><span className={`w-1.5 h-1.5 rounded-full ${call.call_status === "ended" ? "bg-emerald-500" : "bg-amber-400"}`} /><span className="text-gray-600 capitalize">{call.call_status}</span></span></td>
